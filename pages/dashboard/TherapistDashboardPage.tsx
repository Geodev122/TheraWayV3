...
const TherapistDashboardPageShell: React.FC = () => {
    ...
    const fetchDashboardData = useCallback(async () => {
        if (firebaseUser) {
            setIsLoading(true);
            try {
                const therapistDocRef = db.collection('therapistsData').doc(firebaseUser.uid);
                const therapistDocSnap = await therapistDocRef.get();
                ...
                if (therapistDocSnap.exists) {
                    ...
                    await therapistDocRef.set({ ...newTherapist, createdAt: firebase.firestore.FieldValue.serverTimestamp(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
                ...
                const clinicsQuery = db.collection('clinicsData').where('accountStatus', '==', 'live');
                const liveClinicsSnapshot = await clinicsQuery.get();
                ...
                if (liveClinicIds.length > 0) {
                    ...
                    const spacesQuery = db.collection('clinicSpaces').where('clinicId', 'in', liveClinicIds.slice(0,30));
                ...
                const historyCollectionRef = db.collection(`therapistsData/${firebaseUser.uid}/membershipHistory`).orderBy("date", "desc");
                const historyQuerySnapshot = await historyCollectionRef.get(); 
    ...
    const handleProfileSave = async (updatedProfileData: Partial<Therapist>, profilePicFile?: File | null, introVidFile?: File | null) => {
        ...
        try {
            ...
            const therapistDocRef = db.collection('therapistsData').doc(firebaseUser.uid);
            await therapistDocRef.update({ ...dataToSave, updatedAt: firebase.firestore.FieldValue.serverTimestamp()});
    ...
    const handleAccountSave = async (updatedAccount: { name: string; email: string }) => {
        ...
        try {
            ...
            if (therapistData && therapistData.name !== updatedAccount.name) {
                const therapistDocRef = db.collection('therapistsData').doc(firebaseUser.uid);
                await therapistDocRef.update({ name: updatedAccount.name, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    ...
    const handleMembershipApplication = async (receiptFile: File | null) => {
        ...
        try {
            ...
            const therapistDocRef = db.collection('therapistsData').doc(firebaseUser.uid);
            await therapistDocRef.update(applicationDataForFirestore);
            ...
            const historyDocRef = db.collection(`therapistsData/${firebaseUser.uid}/membershipHistory`).doc(historyEntry.id);
            await historyDocRef.set(historyEntry);
    ...
    const handleAddOrUpdateCertification = async (certification: Certification, certFile?: File | null) => {
        ...
        try {
            ...
            const therapistDocRef = db.collection('therapistsData').doc(firebaseUser.uid);
            await therapistDocRef.update({ certifications: updatedCerts, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
