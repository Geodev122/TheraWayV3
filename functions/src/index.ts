...
/**
 * Assigns a default 'client' role to new users upon creation
 * and sets custom claims. Also creates a corresponding user
  ...
  .onCreate(async (user: functions.auth.UserRecord) => {
    try {
      ...
      if (user.email && !user.emailVerified) {
        // Note: This requires Firebase project settings to allow email
        // action links to be sent from a trusted domain (e.g., your
        // Firebase Hosting domain). The actual email sending is handled
        // by Firebase Auth backend.
        try {
      ...
      await userRef.set({
        ...
        email: user.email,
        roles: ["client"],
        // Store roles in Firestore for easier querying/display
      ...
      // You might want to update the user's token to reflect the new
      // claims immediately. This requires the client to re-authenticate or
      // refresh their token. For web clients, a page refresh usually
      // suffices. For mobile, you might need to explicitly call
      // getIdToken(true).
      return true;
    } catch (error) {
...
/**
   ...
   * This function would typically be called via an HTTP request or
   * another trigger from an admin interface.
   *
   * To call this function, an authenticated admin user would make an
   * HTTP POST request to its endpoint with the target userId and
   * newRoles in the request body.
   *
...
export const updateUserRole = functions.https.onCall(
  async (
    data: {userId: string; newRoles: UserRole[]},
    context: functions.https.CallableContext,
  ) => {
    // 1. Authenticate and Authorize: Only allow authenticated admins to
    // call this function
    if (!context.auth) {
    ...
    if (!callerClaims || !callerClaims.roles ||
      !callerClaims.roles.includes(UserRole.ADMIN)) {
      throw new functions.https.HttpsError(
        "permission-denied",
    ...
    // 3. Update Custom Claims
      ...
      console.log(
        `User document roles updated for user ${userId} in Firestore.`,
      );

      return {
        success: true,
        message: `Roles for user ${userId} updated to ${
          newRoles.join(", ")}`,
      };
    } catch (error: unknown) {
